...
function InquiryScreen({ onBack, showToast, onEditOrder }: any) {
  ...
  const getStatusInfo = (status: number, deliveryType?: number | string) => {
    ...
    switch (Number(status)) {
      ...
      case 2:
        return {
          text: isDelivery ? 'جاهز وهو مع شركة التوصيل' : 'جاهز للاستلام من المكتبة',
          bg: '#f3e8ff',
          color: '#6b21a8',
          border: '#d8b4fe',
          desc: isDelivery
            ? 'تم الانتهاء من الطباعة والطلب حالياً جاهز وهو مع شركة التوصيل وفي طريقه إليك.'
...
function OrderForm({ onBack, showToast, initialOrder }: any) {
  ...
  const calculateTotal = () => {
    ...
    const deliveryCost = customerInfo.deliveryType === 'delivery' ? (customerInfo.governorate && customerInfo.governorate !== 'عمان' && customerInfo.governorate !== 'محافظة عمان' ? 4 : 3) : 0;
